...
const RankingList: React.FC<RankingListProps> = ({ items, medals = true, emptyMessage = 'Sem dados', onItemClick }) => {
  if (items.length === 0) {
    return <div style={{ padding: '28px 0', textAlign: 'center', color: 'var(--loja-text-muted, #9B9287)', fontSize: 14 }}>{emptyMessage}</div>;
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      {items.map((item, i) => {
        ...
        const badgeColor = isMedal ? MEDAL_COLORS[i] : 'var(--loja-text-secondary, #6B6258)';
        const badgeBg = isMedal ? MEDAL_BG[i] : 'var(--loja-bg-subtle, #F2EEE2)';
        ...
        return (
          ...
          >
            <div style={{
              ...
              border: isMedal ? `1px solid ${badgeColor}55` : '1px solid var(--loja-border, #E8E2D6)',
            }}>
              {i + 1}
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 8, marginBottom: 5 }}>
                <div style={{ fontSize: 15, fontWeight: 600, color: 'var(--loja-ink, #1C1814)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {item.label}
                  {item.sublabel && <span style={{ fontWeight: 400, color: 'var(--loja-text-muted, #9B9287)', marginLeft: 6, fontSize: 13 }}>{item.sublabel}</span>}
                </div>
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, flexShrink: 0 }}>
                  {item.meta && <span style={{ fontSize: 12, color: 'var(--loja-text-muted, #9B9287)', fontFamily: 'JetBrains Mono, monospace' }}>{item.meta}</span>}
                  <span style={{ fontSize: 15, fontWeight: 600, fontFamily: 'JetBrains Mono, monospace', color: 'var(--loja-ink, #1C1814)' }}>{item.valueLabel}</span>
                </div>
              </div>
