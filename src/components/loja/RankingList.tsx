          ...
          >
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 8, marginBottom: 5 }}>
                <div style={{ fontSize: 15, fontWeight: 600, color: '#1C1814', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {item.label}
                  {item.sublabel && <span style={{ fontWeight: 400, color: '#9B9287', marginLeft: 6, fontSize: 13 }}>{item.sublabel}</span>}
                </div>
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, flexShrink: 0 }}>
                  {item.meta && <span style={{ fontSize: 12, color: '#9B9287', fontFamily: 'JetBrains Mono, monospace' }}>{item.meta}</span>}
                  <span style={{ fontSize: 15, fontWeight: 600, fontFamily: 'JetBrains Mono, monospace', color: '#1C1814' }}>{item.valueLabel}</span>
                </div>
              </div>
              <div style={{ height: 9, borderRadius: 4, background: '#F2EEE6', overflow: 'hidden' }}>
                <div style={{
